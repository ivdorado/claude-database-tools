...
3. Auth type: `sql` (username/password) or `azure-ad-device-code` (interactive MFA login, no password stored)
   - If `sql`: ask for `user` and `password`
...
Warn the user once, briefly, that the password (if any) will pass through this conversation and be stored in plaintext at the per-user config path the CLI reports back — same as if they'd hand-edited the file themselves. If they'd rather not type it here, tell them they can edit the JSON file directly instead (the CLI's output/`client-show` reports its exact path) and stop here.
