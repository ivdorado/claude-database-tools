...
export class ReadData {
  ...
  private validateQuery(query: string): { isValid: boolean; error?: string } {
    ...
    for (const keyword of ReadData.DANGEROUS_KEYWORDS) {
    ...
    for (const pattern of ReadData.DANGEROUS_PATTERNS) {
   ...
   */
  async execute(params: ReadDataParams): Promise<OperationResult> {
    try {
      const { query } = params;
